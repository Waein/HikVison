...
import sdk.HCNetSDK;
import sdk.HCNetSDK.NET_DVR_PTZ_PATTERN;
...
public class PanelPTZ extends JPanel
{
	...
	private void PTZControlAll(int iPTZCommand, int iStop)
	{
		...
		if (!Client.hCNetSDK.NET_DVR_PTZControlWithSpeed(m_lRealHandle, iPTZCommand, iStop, iSpeed))
		{
			//JOptionPane.showMessageDialog(this,"PTZ control fail,error code=" + Client.hCNetSDK.NET_DVR_GetLastError());
			DialogMessage dlg = new DialogMessage("PTZ control fail,error code:"+ Client.hCNetSDK.NET_DVR_GetLastError());
	...
	private void PresetPointActionPerformed(ActionEvent event)
	{
		...
		if (!Client.hCNetSDK.NET_DVR_PTZPreset(m_lRealHandle, HCNetSDK.GOTO_PRESET, iPreset))
		{
			//JOptionPane.showMessageDialog(this,"call preset point fail,error code=" + Client.hCNetSDK.NET_DVR_GetLastError());
			DialogMessage dlg = new DialogMessage("call preset point fail,error code:"+ Client.hCNetSDK.NET_DVR_GetLastError());
			dlg.setBounds(0,0 , 370, 200);
			dlg.setVisible(true);
			return;
		}
	...
	public void SetPresetPointActionPerformed(ActionEvent e)
	{
		Client.GetPTZParam();
		int iPreset = comboBoxPresetPoint.getSelectedIndex() + 1;
		if (!Client.hCNetSDK.NET_DVR_PTZPreset(m_lRealHandle, HCNetSDK.SET_PRESET, iPreset))
		{
			//JOptionPane.showMessageDialog(this,"Set Preset point fail,error code=" + Client.hCNetSDK.NET_DVR_GetLastError());
			DialogMessage dlg = new DialogMessage("Set preset point fail,error code:"+ Client.hCNetSDK.NET_DVR_GetLastError());
			dlg.setBounds(0,0 , 370, 200);
			dlg.setVisible(true);
			return;
		}
	...
	public void DelPresetPointActionPerformed(ActionEvent event)
	{
		Client.GetPTZParam();
		int iPreset = comboBoxPresetPoint.getSelectedIndex() + 1;
		if (!Client.hCNetSDK.NET_DVR_PTZPreset(m_lRealHandle, HCNetSDK.CLE_PRESET, iPreset))
		{
			//JOptionPane.showMessageDialog(this,"delete Preset point fail,error code=" + Client.hCNetSDK.NET_DVR_GetLastError());
			DialogMessage dlg = new DialogMessage("Delete preset point fail,error code:"+ Client.hCNetSDK.NET_DVR_GetLastError());
			dlg.setBounds(0,0 , 370, 200);
			dlg.setVisible(true);
			return;
		}
	...
	public void CruiseRouteActionPerformed(ActionEvent event)
	{
		...
		if (!Client.hCNetSDK.NET_DVR_PTZCruise(m_lRealHandle, HCNetSDK.RUN_SEQ, iCruise, (byte) 0, (short) 0))
		{
			//JOptionPane.showMessageDialog(this,"call cruise route fail,error code=" + Client.hCNetSDK.NET_DVR_GetLastError());
			DialogMessage dlg = new DialogMessage("call cruise route fail,error code:"+ Client.hCNetSDK.NET_DVR_GetLastError());
			dlg.setBounds(0,0 , 370, 200);
			dlg.setVisible(true);
			return;
		}
	...
	public void DelCruiseRouteActionPerformed(ActionEvent event)
	{
		...
		if (!Client.hCNetSDK.NET_DVR_PTZCruise(m_lRealHandle, HCNetSDK.DEL_SEQ, byCruise, (byte) 0, (short) 0))
		{
			//JOptionPane.showMessageDialog(this,"delete cruise route fail,error code=" + Client.hCNetSDK.NET_DVR_GetLastError());
			DialogMessage dlg = new DialogMessage("delete cruise route fail,error code:"+ Client.hCNetSDK.NET_DVR_GetLastError());
			dlg.setBounds(0,0 , 370, 200);
			dlg.setVisible(true);
			return;
		}
	...
	public void StartMemActionPerformed(ActionEvent e)
	{
		...
		if (!Client.hCNetSDK.NET_DVR_RemoteControl(m_lUserID, HCNetSDK.NET_DVR_CONTROL_PTZ_PATTERN, struPTZPattern,
				struPTZPattern.size()))
		{
			//JOptionPane.showMessageDialog(this,"start memory fail,error code=" + Client.hCNetSDK.NET_DVR_GetLastError());
			DialogMessage dlg = new DialogMessage("start memory fail,error code:"+ Client.hCNetSDK.NET_DVR_GetLastError());
			dlg.setBounds(0,0 , 370, 200);
			dlg.setVisible(true);
			return;
		}
	...
	public void StopMemActionPerformed(ActionEvent event)
	{
		...
		if (!Client.hCNetSDK.NET_DVR_RemoteControl(m_lUserID, HCNetSDK.NET_DVR_CONTROL_PTZ_PATTERN, struPTZPattern,
				struPTZPattern.size()))
		{
			//JOptionPane.showMessageDialog(this,"stop memory fail,error code=" + Client.hCNetSDK.NET_DVR_GetLastError());
			DialogMessage dlg = new DialogMessage("stop memory fail,error code:"+ Client.hCNetSDK.NET_DVR_GetLastError());
			dlg.setBounds(0,0 , 370, 200);
			dlg.setVisible(true);
			return;
		}
	...
	public void RunPatternScanActionPerformed(ActionEvent event)
	{
		...
		if (!Client.hCNetSDK.NET_DVR_RemoteControl(m_lUserID, HCNetSDK.NET_DVR_CONTROL_PTZ_PATTERN, struPTZPattern,
				struPTZPattern.size()))
		{
			//JOptionPane.showMessageDialog(this,"start run pattern scan fail,error code=" + Client.hCNetSDK.NET_DVR_GetLastError());
			DialogMessage dlg = new DialogMessage("start run pattern scan ,error code:"+ Client.hCNetSDK.NET_DVR_GetLastError());
			dlg.setBounds(0,0 , 370, 200);
			dlg.setVisible(true);
			return;
		}
	...
	public void StopPatternScanActionPerformed(ActionEvent event)
	{
		...
		if (!Client.hCNetSDK.NET_DVR_RemoteControl(m_lUserID, HCNetSDK.NET_DVR_CONTROL_PTZ_PATTERN, struPTZPattern,
				struPTZPattern.size()))
		{
			//JOptionPane.showMessageDialog(this,"stop run pattern scan fail,error code=" + Client.hCNetSDK.NET_DVR_GetLastError());
			DialogMessage dlg = new DialogMessage("stop run pattern scan ,error code:"+ Client.hCNetSDK.NET_DVR_GetLastError());
			dlg.setBounds(0,0 , 370, 200);
			dlg.setVisible(true);
			return;
		}
	...
	public void DelOnePatterScanActionPerformed(ActionEvent event)
	{
		...
		if (!Client.hCNetSDK.NET_DVR_RemoteControl(m_lUserID, HCNetSDK.NET_DVR_CONTROL_PTZ_PATTERN, struPTZPattern,
				struPTZPattern.size()))
		{
			//JOptionPane.showMessageDialog(this,"delete one pattern scan fail,error code=" + Client.hCNetSDK.NET_DVR_GetLastError());
			DialogMessage dlg = new DialogMessage("delete one pattern scan fail,error code:"+ Client.hCNetSDK.NET_DVR_GetLastError());
			dlg.setBounds(0,0 , 370, 200);
			dlg.setVisible(true);
			return;
		}
	...
	public void DelAllPatterScanActionPerformed(ActionEvent event)
	{
		...
		if (!Client.hCNetSDK.NET_DVR_RemoteControl(m_lUserID, HCNetSDK.NET_DVR_CONTROL_PTZ_PATTERN, struPTZPattern,
				struPTZPattern.size()))
		{
			//JOptionPane.showMessageDialog(this,"delete all pattern scan fail,error code=" + Client.hCNetSDK.NET_DVR_GetLastError());
			DialogMessage dlg = new DialogMessage("delete all pattern scan fail,error code:"+ Client.hCNetSDK.NET_DVR_GetLastError());
			dlg.setBounds(0,0 , 370, 200);
			dlg.setVisible(true);
			return;
		}
